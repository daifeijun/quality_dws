...
public class DateUtil {
    ...
    public static void main(String[] args) throws ParseException {
        System.out.println(getDate("2020-01-01 13:59:59.000",-1));
    }
    /**
     * 将检测时间转成需要计算的时间
     */
    public static String getDate(String time, int code) {
        String date = null;
        try {
            Calendar calendar = Calendar.getInstance();
            calendar.setTime(new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").parse(time));
            calendar.set(Calendar.DATE, calendar.get(Calendar.DATE)+code);//让日期根据code是否减1
